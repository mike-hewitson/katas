...
fn get_demiliter(string_numbers: &str) -> (regex::Regex, usize) {

    let re_custom_delimiter_short = regex::Regex::new(r"//(.)\n").unwrap();
    let re_custom_delimiter_long = regex::Regex::new(r"//(\[.*\])\n").unwrap();
    let re_extract_delimiters = regex::Regex::new(r"\[(.*?)\]").unwrap();
    
    if re_custom_delimiter_short.is_match(string_numbers) {
        let delimiter = re_custom_delimiter_short.captures(string_numbers)
                            .unwrap()
                            .at(1)
                            .unwrap();
        ...
        if re_custom_delimiter_long.is_match(string_numbers) {

            let mut delimiter_list: Vec<&str> = vec![];

            let delimiters = re_custom_delimiter_long.captures(string_numbers)
                                .unwrap()
                                .at(1)
                                .unwrap();

            for cap in re_extract_delimiters.captures_iter(delimiters) {
                delimiter_list.push(cap.at(1).unwrap());
            }

            let re = regex::Regex::new(&delimiter_list.connect("|")).unwrap();

            let start_position = re_custom_delimiter_long.captures(string_numbers)
                                .unwrap()
                                .at(0)
                                .unwrap().len();
...
pub fn add_string(string_numbers: &str) -> i32 {
    ...
    let mut sum: i32 = 0;
    let (re, start_position) = get_demiliter(&string_numbers);
...
mod tests {
    ...
    fn should_panic_when_given_any_negatives() {
        assert_eq!(-3, add_string("//;\n1;-2;3;-4;5;-6"));
    }
    #[test]
    fn should_return_sum_when_given_any_numbers_comma_delim_ignoring_gt_1000() {
        assert_eq!(1001, add_string("1,1000,1001"));
    }
    #[test]
    fn should_return_sum_when_given_custom_delim_ofany_length() {
        assert_eq!(6, add_string("//[abc]\n1abc2abc3"));
    }
    #[test]
    fn should_return_sum_when_given_n_custom_delim_ofany_length() {
        assert_eq!(6, add_string("//[abc][xx]\n1abc2xx3"));
    }
}
