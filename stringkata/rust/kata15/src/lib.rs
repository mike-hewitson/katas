...
fn get_delimiters(string_numbers: &str) -> (regex::Regex, usize) {
    ...
    let re_multi_extract = regex::Regex::new(r"\[(.*?)\]").unwrap();
    ...
    if re_custom_single.is_match(string_numbers) {
        ...
        if re_custom_multi.is_match(string_numbers) {
            ...
            let mut delimiters: Vec<&str> = vec![];

            let extracted_delimiters = re_custom_multi.captures(string_numbers)
                                        .unwrap()
                                        .at(1)
                                        .unwrap();

            for cap in re_multi_extract.captures_iter(extracted_delimiters) {
                delimiters.push(cap.at(1).unwrap());
            }

            let re = regex::Regex::new(&delimiters.connect("|")).unwrap();
...
mod tests {
    ...
    #[test]
    fn should_return_sum_when_given_custom_delimiter() {
    ...
    #[test]
    fn should_support_multi_char_custom_delims() {
        assert_eq!(6, add_string("//[abc]\n1abc2abc3"));
    }
    #[test]
    fn should_support_many_multi_char_custom_delims() {
