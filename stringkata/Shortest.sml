...
This is a shortened version to complete with the one line ruby version:)
*)
...
fun add s = 
	let
		val list_of_ints = List.map (fn x => case Int.fromString x of NONE => 0 | SOME n => n) (String.tokens (fn c => c = (if String.isPrefix "//" s then String.sub(s ,2) else #",") orelse c = #"\n") s)
		val list_of_negatives = List.filter (fn n => n < 0) list_of_ints
	in
		if list_of_negatives <> [] then raise NoNegatives list_of_negatives	else List.foldl (fn (x,acc) => if x > 1000 then acc else x + acc) 0 list_of_ints
	end
